...
export default function App() {
  ...
  const [showPastOctavos, setShowPastOctavos] = useState(false);
  const [showPastOctavosPreds, setShowPastOctavosPreds] = useState(false);
  const [showPastOctavosDetail, setShowPastOctavosDetail] = useState(false);
  const [showPastCuartos, setShowPastCuartos] = useState(false);
  const [showPastCuartosPreds, setShowPastCuartosPreds] = useState(false);
  const [showPastCuartosDetail, setShowPastCuartosDetail] = useState(false);
  const [showPastSemifinales, setShowPastSemifinales] = useState(false);
  const [showPastSemifinalesPreds, setShowPastSemifinalesPreds] = useState(false);
  const [showPastSemifinalesDetail, setShowPastSemifinalesDetail] = useState(false);
  ...
  const renderBumpChart = () => {
    ...
    const filteredMatches = matchesWithOriginalIndex.filter((m) => {
      ...
      if (selectedRoundFilter === "semifinales") return m.stage === "Semifinales";
      if (selectedRoundFilter === "tercer_puesto") return m.stage === "Tercer Puesto";
      if (selectedRoundFilter === "final") return m.stage === "Final";
      return true;
    });
          ...
          <div className="table-card" style={{ padding: "2rem" }}>
            ...
            {adminTab === "matches" && (
              <div className="admin-match-list">
                ...
                {knockoutData?.matches?.filter(m => m.stage === "Tercer Puesto" || m.stage === "Final").map((m) => (
                ...
                {knockoutData?.matches?.filter(m => m.stage === "Semifinales").length > 0 && (
                  <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                    <button
                      type="button"
                      className="past-matches-toggle"
                      onClick={() => setShowPastSemifinales(!showPastSemifinales)}
                    >
                      <span>Fase anterior (Semifinales)</span>
                      <span>{showPastSemifinales ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Semifinales").length})</span>
                    </button>
                    {showPastSemifinales && knockoutData?.matches?.filter(m => m.stage === "Semifinales").map((m) => (
                ...
                {knockoutData?.matches?.filter(m => m.stage === "Cuartos").length > 0 && (
                  <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                    <button
                      type="button"
                      className="past-matches-toggle"
                      onClick={() => setShowPastCuartos(!showPastCuartos)}
                    >
                      <span>Fase anterior (Cuartos)</span>
                      <span>{showPastCuartos ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Cuartos").length})</span>
                    </button>
                    {showPastCuartos && knockoutData?.matches?.filter(m => m.stage === "Cuartos").map((m) => (
                      <div key={m.id} className="admin-knockout-card" style={{ marginTop: "1rem" }}>
                        <div>
                          <span className="match-chip">{m.stage} - Partido {m.id}</span>
                          <div className="admin-team-editors">
                ...
                {knockoutData?.matches?.filter(m => m.stage === "Octavos").length > 0 && (
                  <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                    <button
                      type="button"
                      className="past-matches-toggle"
                      onClick={() => setShowPastOctavos(!showPastOctavos)}
                    >
                      <span>Fase anterior (Octavos)</span>
                      <span>{showPastOctavos ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Octavos").length})</span>
                    </button>
                    {showPastOctavos && knockoutData?.matches?.filter(m => m.stage === "Octavos").map((m) => (
                      <div key={m.id} className="admin-knockout-card" style={{ marginTop: "1rem" }}>
                        <div>
                          <span className="match-chip">{m.stage} - Partido {m.id}</span>
                          <div className="admin-team-editors">
                  ...
                  {knockoutData?.matches?.filter(m => m.stage === "Semifinales").length > 0 && (
                    <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastSemifinalesPreds(!showPastSemifinalesPreds)}
                        style={{ width: "100%" }}
                      >
                        <span>Fase Anterior (Semifinales)</span>
                        <span>{showPastSemifinalesPreds ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Semifinales").length})</span>
                      </button>
                      {showPastSemifinalesPreds && (
                        <div className="knockout-match-grid" style={{ marginTop: "1rem" }}>
                          {knockoutData?.matches?.filter(m => m.stage === "Semifinales").map((m) => (
                            <div key={m.id} className="knockout-match-card">
                              <div className="match-meta">
                                <span>{m.stage}</span>
                                <span>Partido {m.id}</span>
                  ...
                  {knockoutData?.matches?.filter(m => m.stage === "Cuartos").length > 0 && (
                    <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastCuartosPreds(!showPastCuartosPreds)}
                        style={{ width: "100%" }}
                      >
                        <span>Fase Anterior (Cuartos)</span>
                        <span>{showPastCuartosPreds ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Cuartos").length})</span>
                      </button>
                      {showPastCuartosPreds && (
                        <div className="knockout-match-grid" style={{ marginTop: "1rem" }}>
                          {knockoutData?.matches?.filter(m => m.stage === "Cuartos").map((m) => (
                            <div key={m.id} className="knockout-match-card">
                              <div className="match-meta">
                                <span>{m.stage}</span>
                                <span>Partido {m.id}</span>
                  ...
                  {knockoutData?.matches?.filter(m => m.stage === "Octavos").length > 0 && (
                    <div className="past-matches-panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastOctavosPreds(!showPastOctavosPreds)}
                        style={{ width: "100%" }}
                      >
                        <span>Fase Anterior (Octavos)</span>
                        <span>{showPastOctavosPreds ? "Ocultar" : "Mostrar"} ({knockoutData?.matches?.filter(m => m.stage === "Octavos").length})</span>
                      </button>
                      {showPastOctavosPreds && (
                        <div className="knockout-match-grid" style={{ marginTop: "1rem" }}>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "2rem" }}>
              ...
              {participantDetail.knockoutPredictions && participantDetail.knockoutPredictions.length > 0 ? (
                <div>
                  {/* Active Tercer Puesto y Final section */}
                  {participantDetail.knockoutPredictions.filter(m => m.stage === "Tercer Puesto" || m.stage === "Final").length > 0 && (
                    <div className="groups-container" style={{ marginBottom: "1.5rem" }}>
                      <div className="group-card" style={{ gridColumn: "1 / -1" }}>
                        <h3 className="group-title">Tercer Puesto y Final</h3>
                        <div>
                          {participantDetail.knockoutPredictions.filter(m => m.stage === "Tercer Puesto" || m.stage === "Final").map((m) => (
                  ...
                  {/* Collapsible Semifinales section */}
                  {participantDetail.knockoutPredictions.filter(m => m.stage === "Semifinales").length > 0 && (
                    <div className="past-matches-panel" style={{ marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastSemifinalesDetail(!showPastSemifinalesDetail)}
                        style={{ width: "100%", justifyContent: "space-between", display: "flex", alignItems: "center" }}
                      >
                        <span>Fase Anterior (Semifinales)</span>
                        <span>{showPastSemifinalesDetail ? "Ocultar" : "Mostrar"} ({participantDetail.knockoutPredictions.filter(m => m.stage === "Semifinales").length})</span>
                      </button>
                      {showPastSemifinalesDetail && (
                        <div className="groups-container" style={{ marginTop: "1rem" }}>
                          <div className="group-card" style={{ gridColumn: "1 / -1" }}>
                            <h3 className="group-title">Semifinales</h3>
                            <div>
                              {participantDetail.knockoutPredictions.filter(m => m.stage === "Semifinales").map((m) => (
                                <div key={m.matchId} className="match-row">
                                  <div className="team-names">
                                    <div className="team-item">
                                      <span className={`team-name ${m.homeActual > m.awayActual ? "bold" : ""}`}>
                  ...
                  {/* Collapsible Cuartos section */}
                  {participantDetail.knockoutPredictions.filter(m => m.stage === "Cuartos").length > 0 && (
                    <div className="past-matches-panel" style={{ marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastCuartosDetail(!showPastCuartosDetail)}
                        style={{ width: "100%", justifyContent: "space-between", display: "flex", alignItems: "center" }}
                      >
                        <span>Fase Anterior (Cuartos)</span>
                        <span>{showPastCuartosDetail ? "Ocultar" : "Mostrar"} ({participantDetail.knockoutPredictions.filter(m => m.stage === "Cuartos").length})</span>
                      </button>
                      {showPastCuartosDetail && (
                        <div className="groups-container" style={{ marginTop: "1rem" }}>
                          <div className="group-card" style={{ gridColumn: "1 / -1" }}>
                            <h3 className="group-title">Cuartos de Final</h3>
                            <div>
                              {participantDetail.knockoutPredictions.filter(m => m.stage === "Cuartos").map((m) => (
                                <div key={m.matchId} className="match-row">
                                  <div className="team-names">
                                    <div className="team-item">
                                      <span className={`team-name ${m.homeActual > m.awayActual ? "bold" : ""}`}>
                  ...
                  {/* Collapsible Octavos section */}
                  {participantDetail.knockoutPredictions.filter(m => m.stage === "Octavos").length > 0 && (
                    <div className="past-matches-panel" style={{ marginBottom: "1.5rem" }}>
                      <button
                        type="button"
                        className="past-matches-toggle"
                        onClick={() => setShowPastOctavosDetail(!showPastOctavosDetail)}
                        style={{ width: "100%", justifyContent: "space-between", display: "flex", alignItems: "center" }}
                      >
                        <span>Fase Anterior (Octavos)</span>
                        <span>{showPastOctavosDetail ? "Ocultar" : "Mostrar"} ({participantDetail.knockoutPredictions.filter(m => m.stage === "Octavos").length})</span>
                      </button>
                      {showPastOctavosDetail && (
                        <div className="groups-container" style={{ marginTop: "1rem" }}>
                          <div className="group-card" style={{ gridColumn: "1 / -1" }}>
